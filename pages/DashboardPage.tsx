  ...
  const loadCards = async () => {
    try {
      ...
      if (data.length > 0) {
        ...
        const stats = await AnalyticsService.getCardStats(primaryId);

        // Merge stats into the card object for display
        data[0] = {
          ...data[0],
          views: stats.views,
          clicks: stats.clicks,
          // Simple mapping for link analytics
          linkAnalytics: stats.raw.reduce((acc: any, curr: any) => {
            if (curr.event_type === 'click' && curr.metadata?.link_type) {
              const type = curr.metadata.link_type;
        ...
        AnalyticsService.subscribeToAnalytics(primaryId, () => {
          // Re-fetch stats on update
          AnalyticsService.getCardStats(primaryId).then(newStats => {
            setCards(prev => {
              const newCards = [...prev];
              if (newCards[0] && newCards[0].id === primaryId) {
                newCards[0] = {
                  ...newCards[0],
                  views: newStats.views,
                  clicks: newStats.clicks,
                  linkAnalytics: newStats.raw.reduce((acc: any, curr: any) => {
                    if (curr.event_type === 'click' && curr.metadata?.link_type) {
                      const type = curr.metadata.link_type;
                      acc[type] = (acc[type] || 0) + 1;
                    }
